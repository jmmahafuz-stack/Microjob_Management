...
def make_payment(request, pk):
    ...
    if booking.status != 'Completed':
        messages.error(
            request,
            'Payment is allowed only after the service is completed.'
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payment.payment_method = form.cleaned_data.get(
                'payment_method'
            )
            ...
            if payment.transaction_id or payment.receipt:
                ...
                if (
                    ...
                    and not payment.platform_commission
                ):
            ...
            if payment.payment_status == 'Paid':
                messages.success(
                    ...
                    'The worker can now receive their share.'
                )
            else:
                messages.success(
                    request,
